  ...
  <div class="py-12" v-if="template">
    <div class="max-w-5xl mx-auto px-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div><div class="bg-gray-100 rounded-lg overflow-hidden shadow-lg"><img :src="template.preview" :alt="template.name" class="w-full" /></div></div>
        <div>
          ...
          <div class="flex flex-wrap gap-2 mb-6">
            ...
            <span class="px-3 py-1 rounded-full bg-gray-100 text-gray-600 text-sm">{{ template.slides }} {{ t('detail.slides') }}</span>
          </div>
          <p class="text-gray-700 mb-6">{{ template.description }}</p>
          <h3 class="font-semibold mb-3">{{ t('detail.features') }}</h3>
          <ul class="space-y-2 mb-8">
            <li v-for="f in template.features" :key="f" class="flex items-center text-gray-700"><Check class="w-5 h-5 text-green-500 mr-2 flex-shrink-0" />{{ f }}</li>
          </ul>
          <button @click="download" class="w-full bg-blue-600 text-white py-4 rounded-lg font-semibold text-lg hover:bg-blue-700 mb-3 cursor-pointer">{{ t('detail.download') }}</button>
      ...
      <div class="mt-16">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <TemplateCard v-for="tpl in related" :key="tpl.id" :template="tpl" />
        </div>
      </div>
    </div>
  </div>
...
function download() { if (template.value) window.open(template.value.downloadUrl, '_blank') }
</script>
