<template>
  <div class="py-12">
    <div class="max-w-7xl mx-auto px-4">
      ...
      <p class="text-gray-600 mb-8">{{ t('templates.subtitle') }}</p>
      <div class="flex flex-wrap gap-3 mb-8">
        ...
        <button v-for="cat in cats" :key="cat" @click="selectedCategory = cat" :class="selectedCategory === cat ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'" class="px-4 py-2 rounded-full text-sm font-medium">{{ cat }}</button>
      </div>
...
const { t } = useI18n()
const cats = ['Business', 'Creative', 'Academic', 'Technology', 'Marketing']
